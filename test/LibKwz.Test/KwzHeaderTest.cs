...
{
    private readonly byte[] sampleHeader =
    [
    ...
    {
        using var stream = new MemoryStream(sampleHeader);
    ...
    public void TestSettingFlags()
    {
        ...
        header.Locked = true;
        header.LoopPlayback = true;
        header.Toolset = true;

        Assert.NotEqual(flags, header.Flags);

        header.Locked = false;
        header.LoopPlayback = true;
        header.Toolset = false;

        Assert.Equal(flags, header.Flags);
    }

    [Fact]
    public void TestSettingLayerVisibilityFlags()
    {
        var header = KwzHeader.ReadFrom(new BinaryReader(new MemoryStream(sampleHeader)));
        Assert.Equal(0, header.LayerVisibilityFlags);

        var layerVisibilityFlags = header.LayerVisibilityFlags;

        header.LayerAVisible = true;
        header.LayerBVisible = true;
        header.LayerCVisible = true;

        Assert.Equal(layerVisibilityFlags, header.LayerVisibilityFlags);

        header.LayerAVisible = false;
        header.LayerBVisible = true;
        header.LayerCVisible = false;
